...
impl ReadTools {
    ...
    pub fn list_bundles(&self) -> Result<Vec<BundleInfo>, String> {
        Ok(self.bundles.iter().map(|(name, repo)| {
            BundleInfo {
                name: name.clone(),
    ...
    pub fn list_concepts(&self, bundle: &str, prefix: Option<&str>, type_filter: Option<&str>, tag_filter: Option<&str>) -> Result<Vec<ConceptId>, String> {
        let repo = self.get_bundle(bundle)?;
        repo.list_concepts(prefix, type_filter, tag_filter).map_err(|e| e.to_string())
    }
    ...
    pub fn read_index(&self, bundle: &str, path: &str) -> Result<crate::bundle::repo::IndexReadResult, String> {
        let repo = self.get_bundle(bundle)?;
    ...
    pub fn get_graph(&self, bundle: &str, prefix: Option<&str>) -> Result<crate::bundle::repo::GraphResult, String> {
        let repo = self.get_bundle(bundle)?;
        repo.get_graph(prefix).map_err(|e| e.to_string())
    }

    pub fn search(&self, bundle: &str, query: &str, type_filter: Option<&str>, tag_filter: Option<&str>) -> Result<Vec<SearchResult>, String> {
        let repo = self.get_bundle(bundle)?;
        repo.search(query, type_filter, tag_filter).map_err(|e| e.to_string())
    }
